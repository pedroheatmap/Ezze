...
def calcular_distancia_lote(lote_clientes, oficinas, frequencias=None):
    ...
    distancias = 6371 * 2 * np.arctan2(np.sqrt(a), np.sqrt(1 - a))
    
    # Aplicar frequências se fornecidas (modificado para multiplicar corretamente)
    if frequencias is not None:
        distancias = distancias * frequencias[np.newaxis, :]
...
# Na seção de carregamento de dados, modifique para:
...
try:
    ...
    # Verifica se a coluna Frequencia existe, senão cria com valor 1
    if 'Frequencia' not in clientes_df_raw.columns:
        clientes_df_raw['Frequencia'] = 1
    ...
    # Obter frequências como array numpy
    frequencias_clientes = clientes_df['Frequencia'].values.astype(np.float32)
    ...
    print("\n📌 Amostra de oficinas:")
    print(oficinas_df[['Nome', 'Latitude', 'Longitude', 'Rede Car10']].head(3))
    print("\n📌 Amostra de clientes:")
    print(clientes_df[['Latitude', 'Longitude', 'Frequencia']].head(3))
...
<body>
    ...
    <script>
        ...
        div.innerHTML = `
        <div><i style="background:#e74c3c; width:12px; height:12px; display:inline-block; border-radius:50%;"></i> Oficina Car10</div>
        <div><i style="background:#27ae60; width:12px; height:12px; display:inline-block; border-radius:50%;"></i> Oficinas Ezze</div>
    ...
    return div;
                            ...
                            [oficina.lat, oficina.lon], 
        {
            ...
            fillColor: oficina.cor || '#27ae60',  // Usa a cor definida ou verde padrão
            color: '#fff',
            weight: 1,
            fillOpacity: 0.8
        }
        ...
        <div class="popup-content">
            ...
            🔴 <b>Raio:</b> ${raio} km<br>
            👥 <b>Clientes:</b> ${oficina.clientes.toLocaleString()}<br>
            ${oficina.cor === '#e74c3c' ? '🔴 <b>Rede Car10:</b> Sim' : '🟢 <b>Rede Car10:</b> Não'}
        </div>
    `).addTo(marcadores);
});
                   ...
                   document.getElementById('descobertos').textContent = (data.total_clientes - data.clientes_cobertos).toLocaleString();
                   document.getElementById('percentual').textContent = `${data.percentual_coberto.toFixed(1)}%`;
                    // Ajustar zoom
...
# Na rota /cobertura, modifique o cálculo de clientes cobertos:
...
def obter_cobertura():
    try:
        ...
        clientes_cobertos = 0
        clientes_por_oficina = np.zeros(len(oficinas_df), dtype=np.float32)
        
        # Array para marcar clientes já cobertos (evitar dupla contagem)
        clientes_marcados = np.zeros(len(coords_clientes), dtype=bool)
        ...
        for i in range(0, len(coords_clientes), LOTE_CLIENTES):
            ...
            # Marcar clientes cobertos por pelo menos uma oficina
            cobertura_lote = np.any(distancias <= raio, axis=0)
            clientes_marcados[i:i+len(lote)] |= cobertura_lote
            
            # Clientes por oficina (dentro do raio, considerando frequência)
            clientes_por_oficina += np.sum((distancias <= raio) * frequencias[np.newaxis, :], axis=1)
        
        # Calcular total de clientes cobertos (sem dupla contagem)
        clientes_cobertos = np.sum(frequencias_clientes[clientes_marcados])
        total_clientes = np.sum(frequencias_clientes)
        
        # Preparar resposta
        ativas = []
        for idx, row in oficinas_df.iterrows():
            if idx < len(clientes_por_oficina):
                cor = '#e74c3c' if row.get('Rede Car10', '').lower() == 'sim' else '#27ae60'
                ativas.append({
                    'id': idx + 1,
                    'nome': row.get('Nome', f'Oficina {idx+1}'),
        ...
        return jsonify({
            ...
            'total_clientes': int(total_clientes),  # Adicionado para referência
            'percentual_coberto': float(clientes_cobertos / total_clientes * 100),
...
def sugerir_oficinas():
    try:
        ...
        # 1. Identificar clientes NÃO cobertos pelas oficinas existentes (considerando frequência)
        mascara_nao_cobertos = np.ones(len(coords_clientes), dtype=bool)
        ...
        # 3. Calcular centroides e número de clientes por cluster (considerando frequência)
        sugestoes = []
        ...
        for cluster_id in np.unique(clusters):
            ...
            freq_cluster = frequencias_nao_cobertos[mascara_cluster]
...
def clientes_heatmap():
    try:
        ...
        # Usar frequência como intensidade base
        coords = clientes_sample[['Latitude', 'Longitude']].values
        intensities = clientes_sample['Frequencia'].values * 0.8  # Valor base proporcional à frequência
